...
pkgAdPantallas = packages.PkgAdPantallas()


class DefinicionPantallasFormulariosView(TemplateView):

    template_name = 'ADM_0018.html'
    @method_decorator(csrf_exempt)
    def dispatch(self, request, *args, **kwargs):
        self.cod_empresa = self.request.session.get('VS_COD_EMPRESA')
        self.tbPantallas = 'TablaPantallas'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'searchdata_pantallas':
                with connection.cursor() as cursor:
                    cursor.callproc(pkgAdPantallas.consultar, [None, None, None, None])
                    rows = cursor.fetchall()
                    pantallas = [dict(zip([column[0] for column in cursor.description], row)) for row in rows]
                    return JsonResponse(pantallas, safe=False)
    ...
    def process_added_record(self, table_id, record):
        with connection.cursor() as cursor:
            if table_id == self.tbPantallas:
                record_to_insert = (
                    record['cod_pantalla'],
                    record['nombre_pantalla'],
                    record['ruta'],
                    None,
                    None,
                    None,
                    None,
                    record['manual_proceso'],
                    # record['audit_ip_ing'],
                    None,
                    None,
                )

                mogrified_value = cursor.mogrify("(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", record_to_insert).decode('utf-8')
                cursor.execute(pkgAdPantallas.insertar % mogrified_value)
    ...
    def process_updated_record(self, table_id, record):
        with connection.cursor() as cursor:
            if table_id == self.tbPantallas:
                ...
                mogrified_value = cursor.mogrify("(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", record_to_insert).decode('utf-8')
                cursor.execute(pkgAdPantallas.modificar % mogrified_value)
                result = cursor.fetchall()
                print('Insert result:', result)
    ...
    def process_deleted_record(self, table_id, record):
        with connection.cursor() as cursor:
            if table_id == self.tbPantallas:
                ...
                mogrified_value = cursor.mogrify("(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", record_to_insert).decode('utf-8')
                cursor.execute(pkgAdPantallas.borrar % mogrified_value)
                result = cursor.fetchall()
                print('Insert result:', result)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['titulo'] = 'Listado de Pantallas '
        context['lista_url'] = reverse_lazy('definicion-pantallas-formularios')
        # context['entity'] = 'Tipo'
